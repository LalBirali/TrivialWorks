...
//Tests default behavior of our KV Store: Ensuring DF is distributed 
class Test {
    public:
    Sys sys;
     ...
     **/
    void testDistribution(KVStore * kv) {
        vector<DataFrame*> arr = kv->dfs;
        for (int i = 0; i < arr.size(); i++) {
            sys.t_true(arr.at(i)->nrows() == 52);
        }
        String* t1 = arr.at(1)->get_string(1,17);
        String* t2 = new String("23");
        sys.t_true(t1->equals(t2));
        sys.pln("Values distribted correctly -- TESTS PASS.");
    }
